 ...
 *  Loongson Inter-Processor Interrupts support
 */
...
#define ipi_iocsr_writeq(val, reg)	iocsr_write64(val, reg)
#define ipi_iocsr_readq(reg)		iocsr_read64(reg)
#endif
...
static DECLARE_BITMAP(ipi_allocated, IPI_INT_COUNT);
static int ipi_parent_irq;

DEFINE_STATIC_KEY_TRUE(loongson_ipi_iocsr);

/*
 * As per loongsonseries_FAQ_V1.1, write to EN and SET must be
 * followed by a immediate read to workaround a hardware issue.
 * This does not affect IOCSR based hardware.
 */
static __always_inline void ipi_writel(u32 val, void __iomem *addr)
{
...
static void ipi_mmio_handle(struct irq_desc *desc)
{
	...
	for_each_set_bit(hwirq, &status, IPI_INT_COUNT)
...
static void ipi_mmio_mask(struct irq_data *d)
{
	...
	val = readl(ipi_base[smp_processor_id()] + IPI_REG_EN);
	val &= ~BIT(irqd_to_hwirq(d));
	ipi_writel(val, ipi_base[smp_processor_id()] + IPI_REG_EN);
}
...
static void ipi_mmio_unmask(struct irq_data *d)
{
	...
	val = readl_relaxed(ipi_base[smp_processor_id()] + IPI_REG_EN);
	val |= BIT(irqd_to_hwirq(d));
	ipi_writel(val, ipi_base[smp_processor_id()] + IPI_REG_EN);
}
...
static void ipi_mmio_ack(struct irq_data *d)
{
	...
	val = BIT(irqd_to_hwirq(d));
	ipi_writel(val, ipi_base[smp_processor_id()] + IPI_REG_CLR);
}

static void ipi_mmio_send_single(struct irq_data *d, unsigned int cpu)
{
	ipi_writel(BIT(irqd_to_hwirq(d)), ipi_base[cpu] + IPI_REG_SET);
}
...
static const struct irq_chip ipi_mmio_chip = {
	...
	.ipi_send_single	= ipi_mmio_send_single,
};
...
static void ipi_iocsr_send_single(struct irq_data *d,
				unsigned int cpu)
{
	u32 val;
...
static const struct irq_chip ipi_iocsr_chip = {
	...
	.ipi_send_single	= ipi_iocsr_send_single,
};
...
static int ipi_cpu_starting(unsigned int cpu)
{
	if (loonsgon_ipi_is_iocsr()) {
		...
		ipi_writel(0x0, ipi_base[cpu] + IPI_REG_EN);
		ipi_writel(0xffffffff, ipi_base[cpu] + IPI_REG_CLR);
	}
			    ...
			    unsigned int nr_irqs, void *arg)
{
	...
	i = bitmap_weight(ipi_allocated, IPI_INT_COUNT);
	if (i + nr_irqs > IPI_INT_COUNT)
		return -ENOSPC;

	for (i = 0; i < nr_irqs; i++) {
		int hwirq = find_first_zero_bit(ipi_allocated, IPI_INT_COUNT);
...
static int __init ipi_irq_domain_init(struct fwnode_handle *fwnode)
{
	...
	domain = irq_domain_create_linear(fwnode, IPI_INT_COUNT,
					  &ipi_domain_ops, NULL);
	if (!domain)
		return -ENOMEM;
